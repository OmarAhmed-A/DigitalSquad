...
def GoToExit(steps_back, rescue_items):
  ...
  print ("score: ", score_now, goto_exit)
...
def local_inference(riddle_solvers):
    ...
    for t in range(300):
      ...
      x = [state_0[0][0], state_0[0][1]]
      
      if (reach_END and not np.array_equal(obv[0], (9,9)) and back == 0 ):
        backToEnd.append(x)
      
      print("back to end",reach_END is True , GoToExit( len(backToEnd), info['rescued_items']) , info['rescued_items'] >1,  len(backToEnd))
      if (not info['riddle_type'] == None):
        print("oooh")
        
      # print (reach_END, GoToExit( len(backToEnd), info['rescued_items']), )
      print(backToEnd)
      if(reach_END is True and info['rescued_items'] >1 and len(backToEnd) > 0 and not info['riddle_type'] == None or back == 1 ):
        print ("hanet")
        ...
        if (GoToExit( len(backToEnd), info['rescued_items']) == 1):
          
          print(state_0[0], backToEnd[i])
          ...
          print(backToEnd)
          i += 1
          if (state_0[0][0] == backToEnd[i][0] and state_0[0][1] == backToEnd[i][1]):
            i +=1
          x = state_0[0][0] - backToEnd[i][0]
          y = state_0[0][1] - backToEnd[i][1]
          print(state_0[0])
          print(x,y)
          Rback = idx.index((x,y))
          print (Rback)
          ...
          if np.array_equal(obv[0], (9,9)): # Exit at (9,9) with 3 or more
              logging.debug ("out with back desicion")
              manager.set_done(agent_id)
              break
        else:
          action, action_index = select_action(state_0) 
      ...
      elif (steps > 51 and back == 0):
        if (steps > 50 and steps < 160): # Worst case with 3 riddles (16 PT)
          # Select an action
        ...
        #   End_path.reverse()
        #   for p in End_path:
        #     action, action_index = p, actions.index(p) 
        #     obv, reward, terminated, truncated, info = manager.step(agent_id, action)
        # # THIS IS A SAMPLE TERMINATING CONDITION WHEN THE AGENT REACHES THE EXIT
          # print(visited)
        elif(steps > 159 and steps < 200): # 4 riddles (Worst = 16 PT)
          
          action, action_index = select_action(state_0) 
          obv, reward, terminated, truncated, info = manager.step(agent_id, action)
          print ("resude from info 0.1",info['rescued_items'])
          if not info['riddle_type'] == None:
              solution = riddle_solvers[info['riddle_type']](info['riddle_question'])
              obv, reward, terminated, truncated, info = manager.solve_riddle(info['riddle_type'], agent_id, solution)
          ...
          if ((9,9) in N_Visited):
            if (info['rescued_items'] > 2 and GoToExit(len(backToEnd), info['rescued_items']) == 0 and not info['riddle_type'] == None ):
                print ("a7a")
                if ():
                  print ("yaalaa")
                  manager.set_done(agent_id)
              ...
              if not info['riddle_type'] == None:
                solution = riddle_solvers[info['riddle_type']](info['riddle_question'])
                obv, reward, terminated, truncated, info = manager.solve_riddle(info['riddle_type'], agent_id, solution)
              print ("resude from info 0.2",info['rescued_items'])
          elif ((9,9) not in N_Visited):
            
            action, action_index = select_action(state_0) 
            obv, reward, terminated, truncated, info = manager.step(agent_id, action)
            print ("resude from info 0.3",info['rescued_items'])
            if not info['riddle_type'] == None:
                solution = riddle_solvers[info['riddle_type']](info['riddle_question'])
                obv, reward, terminated, truncated, info = manager.solve_riddle(info['riddle_type'], agent_id, solution)
        ...
        action, action_index = select_action(state_0) 
        obv, reward, terminated, truncated, info = manager.step(agent_id, action)
        print("resude from info 0.4",info['rescued_items'])
        if not info['riddle_type'] == None:
            solution = riddle_solvers[info['riddle_type']](info['riddle_question'])
            obv, reward, terminated, truncated, info = manager.solve_riddle(info['riddle_type'], agent_id, solution)
        # if np.array_equal(obv[0], (9,9)):
